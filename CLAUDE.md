...
This is a **Claude Code plugin** — not a software package with a build step. It provides a drop-in long-term memory system for Claude Code projects: raw session notes are recorded during work and later distilled into a curated Obsidian vault via the `/claude-project-memory:digest` command.
...
2. Update the skills table in `skills/digest-daily/SKILL.md`
...
Work session → memory/daily/YYYY-MM-DD_HHMMSS.md   (raw log)
              ...
              docs/vault/**/*.md                     (curated knowledge)
                       ↓ /claude-project-memory:search sub-agent
...
| `skills/digest/SKILL.md`              | `/claude-project-memory:digest` slash command in plugin format              |
| `skills/digest-daily/SKILL.md`        | Skill used as sub-agent: distills one daily log → vault (uses Sonnet)       |
| `skills/digest-spec/SKILL.md`         | Skill used as sub-agent: distills one spec → vault (uses Sonnet)            |
| `skills/digest-rules/SKILL.md`        | Shared vault-writing rules invoked by both digest sub-agents                |
| `skills/search/SKILL.md`              | Skill used as sub-agent: retrieves vault docs before tasks (uses Haiku)     |
| `skills/install/SKILL.md`             | `/claude-project-memory:install` slash command: installs the plugin into the current project |
| `.claude-plugin/marketplace.json`     | Plugin marketplace registration                                             |
| `skills/memory/SKILL.md`              | Operating instructions for what/when to record                              |
...
- `memory_search_reminder.py` — `UserPromptSubmit`: suggests invoking `claude-project-memory:search` before non-trivial tasks
...
### `/claude-project-memory:digest` Pipeline

Processes files **sequentially** (not in parallel) to avoid write conflicts in shared vault documents like `Decisions/Index.md`:

1. Find all `memory/daily/*.md` → read `skills/digest-daily/SKILL.md` → spawn `general-purpose` agent with skill content + file path → delete on success
2. Find all `specs/*.md` not in `specs/digested.txt` → read `skills/digest-spec/SKILL.md` → spawn `general-purpose` agent with skill content + file path → append to `digested.txt` on success
